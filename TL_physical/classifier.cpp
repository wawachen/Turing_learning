...
void CF_elmanNetwork(const double CF_input[], const unsigned& C_num)
{
    ...
    for (unsigned i = 0; i < CF_hidden_n; i++)
    {
        ...
        {
            CF_hidden[i] += classifier[C_num].chrom[i * CF_input_n + j] * CF_input[j]; //weight * input
        }
    }
    ...
    for (unsigned i = 0; i < CF_hidden_n; i++)
    {
        ...
        {
            CF_hidden[i] += classifier[C_num].chrom[offset + i * CF_hidden_n + j] * pastCF_hidden[j]; //weight * input
        }
    }
    ...
    {
        CF_output[i] = classifier[C_num].chrom[offset + i * (CF_hidden_n + 1)] * (1.0);   //add a bias
        for (unsigned j = 0; j < CF_hidden_n; j++)
        {
            CF_output[i] += classifier[C_num].chrom[offset + i * (CF_hidden_n + 1) + (j + 1)] * CF_hidden[j]; //weight * input
        }
    }
...
void generate_initial_classifiers()
{
    for(unsigned i = 0; i < CLASSIFIER_POPSIZE; i++)
    {
        ...
        {
            classifier[i].chrom[j] = gsl_ran_flat(r, -10.0,10.0);
